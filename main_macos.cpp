...
#include <stdio.h>
#include <stdbool.h>
#include <GLUT/glut.h> // main change for MacOS. please install with xcode tools. 
#include <time.h>
...
void draw_player() {
    GLfloat object_diffuse[] = { 1.0, 1.0, 1.0, 1.0 }; // Diffuse color for the objects (white)
...
void init() {
    ...
    glEnable(GL_LIGHT0);
    GLfloat light_position[] = { 0.0, 0.0, 1.0, 0.0 }; // Positional light at (0, 0, 1)
    GLfloat light_diffuse[] = { 0.0, 0.0, 1.0, 1.0 }; // Diffuse color (white)
    GLfloat light_ambient[] = { 0.2, 0.2, 0.2, 1.0 }; // Ambient color (gray)
